                ...
                DecoderState::Body { message } => {
                    ...
                    match message_ref.header_type() {
                        ...
                        MessageHeader::RecordBatch => {
                            ...
                            let ipc_schema_message = self
                                .ipc_schema_message.clone()
                                .ok_or_else(|| ArrowError::IpcError("Missing IPC schema".to_string()))?;
                            let ipc_schema = ipc_schema_message.as_ref().as_ref().header_as_schema().unwrap();

                            let schema = self
                                .schema
                                .clone()
                                .ok_or_else(|| ArrowError::IpcError("Missing schema".to_string()))?;
