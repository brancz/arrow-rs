    ...
    fn create_array(
        ...
        match data_type {
            ...
            List(ref list_field) | LargeList(ref list_field) | Map(ref list_field, _) => {
                ...
                let values = self.create_array(ipc_field.children().unwrap().get(0), list_field, variadic_counts)?;
            ...
            Struct(struct_fields) => {
                ...
                for (idx, struct_field) in struct_fields.iter().enumerate() {
                    let child = self.create_array(ipc_field.children().unwrap().get(idx), struct_field, variadic_counts)?;
            ...
            RunEndEncoded(run_ends_field, values_field) => {
                ...
                let run_ends = self.create_array(children.get(0), run_ends_field, variadic_counts)?;
        ...
        if let Some(projection) = self.projection {
            ...
            for idx in 0..ipc_fields_len {
                ...
                if let Some(proj_idx) = projection.iter().position(|p| p == &idx) {
                    let child = self.create_array(ipc_fields.get(idx), self.schema.clone().field(idx), &mut variadic_counts)?;
...
fn first_field_with_dict_id_from_schema(
    schema: crate::Schema,
    id: i64,
) -> Option<Field> {
...
fn first_field_with_dict_id_from_field(
    field: crate::Field,
    id: i64,
) -> Option<Field> {
...
fn read_dictionary_impl(
    ...
    let dictionary_values: ArrayRef = match first_field.data_type() {
        DataType::Dictionary(_, ref value_type) => {
            ...
            let data = gen.schema_to_bytes_with_dictionary_tracker(&schema, &mut dict_tracker, &IpcWriteOptions::default());
...
impl FileDecoder {
    ...
    pub fn new(footer_buffer: Vec<u8>, verifier_options: VerifierOptions, version: MetadataVersion) -> Self {
        let footer = crate::root_as_footer_with_opts(&verifier_options, &footer_buffer[..]).unwrap();
    ...
    pub fn read_dictionary(&mut self, block: &Block, buf: &Buffer) -> Result<(), ArrowError> {
        ...
        match message.header_type() {
            crate::MessageHeader::DictionaryBatch => {
                ...
                let footer = crate::root_as_footer_with_opts(&self.verifier_options, &self.footer_buffer[..]).unwrap();
                let ipc_schema = footer.schema().unwrap();
                read_dictionary_impl(
...
impl FileReaderBuilder {
    ...
    pub fn build<'a, R: Read + Seek>(self, mut reader: R) -> Result<FileReader<R>, ArrowError> {
    ...
    fn maybe_next(&mut self) -> Result<Option<RecordBatch>, ArrowError> {
        ...
        match message.header_type() {
            ...
            crate::MessageHeader::RecordBatch => {
                ...
                let message = crate::root_as_message(&self.schema_message_buffer).map_err(|err| {
                    ArrowError::ParseError(format!("Unable to get root as message: {err:?}"))
                })?;
                // message header is a Schema, so read it
                let ipc_schema: crate::Schema = message.header_as_schema().ok_or_else(|| {
...
mod tests {
    ...
    fn read_ipc_with_decoder_inner(
        ...
        let mut decoder = unsafe {
            FileDecoder::new(buffer[trailer_start - footer_len..trailer_start].to_vec(), Default::default(), footer.version())
                .with_skip_validation(skip_validation)
        };
        // Read dictionaries
        for block in footer.dictionaries().iter().flatten() {
    ...
    fn test_unaligned() {
        ...
        let encoded_schema = gen.schema_to_bytes_with_dictionary_tracker(&batch.schema(), &mut dict_tracker, &IpcWriteOptions::default());
